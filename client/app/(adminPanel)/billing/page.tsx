...
async function fetchBills(searchParams: { page?: string; limit?: string }) {
...
export default async function BillsPage({
  ...
  const result = await fetchBills(searchParams);
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <div className="flex gap-2">
          <Link
            href={`/billing?page=${Math.max(1, pagination.currentPage - 1)}&limit=${pagination.limit}`}
            className="px-3 py-2 rounded border border-gray-200 hover:bg-gray-50 text-sm"
          >
            Prev
          </Link>
          <Link
            href={`/billing?page=${Math.min(pagination.totalPages, pagination.currentPage + 1)}&limit=${pagination.limit}`}
            className="px-3 py-2 rounded border border-gray-200 hover:bg-gray-50 text-sm"
          >
            Next
          </Link>
