        ...
        setCart(prevCart => {
            ...
            if (existingItem) {
                return prevCart.map(cartItem =>
                    cartItem.uniqueId === uniqueId ? { ...cartItem, quantity: cartItem.quantity + 1 } : cartItem
                );
            } else {
                return [...prevCart, {
                    uniqueId,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 bg-gray-50 min-h-screen">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {menuItems.map(item => (
                        <div key={item.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col justify-between">
                            <div>
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                {item.unit && item.unit.length > 0 ? (
                                    item.unit.map(u => (
                                        ...
                                        >
                                            <Plus size={14} />
                                            {u.unit} <span className="text-xs text-orange-500">${u.price}</span>
                                        </button>
                                    ))
                                ) : (
                                    <p className="text-sm text-red-400 italic">No units available</p>
                                )}
                            </div>
                        </div>
                    ))}
            ...
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-6 self-start h-[calc(100vh-3rem)] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-gray-200">
                    {cart.length === 0 ? (
                        ...
                        cart.map(item => (
                            <div key={item.uniqueId} className="flex flex-col bg-gray-50 rounded-lg p-3 relative group">
                                <button 
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 uppercase font-semibold tracking-wide">{item.unitName}</p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 bg-white px-2 py-1 rounded-md shadow-sm border border-gray-200">
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                const unit = menuItem?.unit.find(u => u.unit === item.unitName);
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all animate-in fade-in zoom-in-95 duration-200">
                        <div className="text-center mb-6">
                            ...
                            <p className="text-gray-500 mt-2">Are you sure you want to place this order?</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <button 
                                ...
                                disabled={loading}
                                className="px-4 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-colors flex items-center justify-center gap-2"
                            >
