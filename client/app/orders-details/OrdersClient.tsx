...
import { Loader2, Search, ChevronLeft, ChevronRight, CheckCircle, XCircle, Clock, Eye, X } from "lucide-react";
...
export default function OrdersClient({ initialOrders, pagination }: OrdersClientProps) {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    try {
      ...
      await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8080"}/api/orders/${orderId}/status`,
        { status: newStatus },
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 relative">
      ...
      <div className={`bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden relative transition-opacity duration-200 ${isPending ? 'opacity-60 pointer-events-none' : ''}`}>
              ...
              {initialOrders.map((order) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 max-w-xs truncate text-gray-600">
                    {order.items.length} items ({order.items.map(i => i.menuItem.name).slice(0, 2).join(", ")}{order.items.length > 2 ? "..." : ""})
                  </td>
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white/80 backdrop-blur-md">
              <div>
                ...
                <span className="text-xs font-mono text-gray-400">#{selectedOrder.id}</span>
              </div>
              <button 
            ...
            <div className="p-6 space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <div>
                      ...
                      <div className="font-medium text-gray-900">{selectedOrder.user?.firstName} {selectedOrder.user?.lastName}</div>
                      <div className="text-sm text-gray-600">{selectedOrder.user?.email}</div>
                  </div>
                  <div>
                      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Order Info</h3>
                                      ...
                                      <td className="px-4 py-3">
                                          <div className="flex items-center gap-3">
                                              {item.menuItem.imageUrl && (
                                                <img src={item.menuItem.imageUrl} alt="" className="h-10 w-10 rounded-md object-cover bg-gray-100" />
                                              )}
